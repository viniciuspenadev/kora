...
// Install id (device trust F6): identidade PERSISTENTE desta instalação — vira
// auth_devices kind='extension' no servidor. Gerado uma vez, nunca some no
// logout (dispositivo ≠ sessão). Instalação nova = código por e-mail.
async function getDeviceKey() {
  const s = await chrome.storage.local.get(["deviceKey"])
  if (s.deviceKey) return s.deviceKey
    ...
    switch (msg && msg.type) {
      ...
      case "login": {
        ...
        const r = await api("/api/ext/auth", {
          method: "POST",
          body: JSON.stringify({
            email: msg.email, password: msg.password, label: msg.label || "Chrome",
        ...
        // code "device_challenge" passa direto pro sidebar → tela de código.
        sendResponse(r)
        break
      }
      case "loginVerify": {
        // Etapa 2 (device trust F6): código do e-mail → confiança 30d → token.
        const r = await api("/api/ext/auth/verify", {
          method: "POST",
          body: JSON.stringify({
            email: msg.email, code: msg.code, label: msg.label || "Chrome",
            deviceKey: await getDeviceKey(),
          }),
        })
        if (r.ok) await chrome.storage.local.set({ token: r.data.token, user: r.data.user, tenant: r.data.tenant })
