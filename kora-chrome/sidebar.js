...
async function onLogin(ev) {
  ...
  // Dispositivo não reconhecido (device trust F6): o servidor mandou um código
  // de 6 dígitos pro e-mail — troca pra tela de verificação.
  if (r && !r.ok && r.code === "device_challenge") {
    holdRender = false
    renderLoginCode(email)
    return
  }
...
// ── Etapa 2 do login (device trust F6): código do e-mail ─────────
// Mesma anatomia da tela de login: logo + título + 1 input + CTA único +
// erro inline com shake. O install id (deviceKey) endereça o desafio no server.
function renderLoginCode(email) {
  setView(`
    <div class="login">
      <img src="assets/logo.svg" alt="Kora" class="login-logo" />
      <div class="login-title">Confirme este dispositivo</div>
      <div class="hint" style="text-align:center;margin-bottom:12px;">
        Não reconhecemos esta instalação. Enviamos um código de 6 dígitos para
        <b>${esc(email)}</b>.
      </div>
      <form id="code-form">
        <label>Código:
          <input id="lg-code" type="text" inputmode="numeric" autocomplete="one-time-code"
                 maxlength="6" placeholder="••••••" required
                 style="text-align:center;letter-spacing:8px;font-weight:700;" />
        </label>
        <div id="code-err" class="err" hidden></div>
        <button id="code-btn" class="btn btn-primary" type="submit">Confirmar dispositivo</button>
      </form>
      <div class="hint" style="text-align:center;margin-top:12px;">
        <a class="lnk" id="code-back" href="#">Voltar pro login</a>
      </div>
    </div>`)
  view.classList.remove("canvas")
  document.getElementById("code-back").addEventListener("click", (e) => { e.preventDefault(); renderLogin() })
  document.getElementById("lg-code").addEventListener("input", (e) => {
    e.target.value = e.target.value.replace(/\D/g, "").slice(0, 6)
  })
  document.getElementById("code-form").addEventListener("submit", (ev) => onLoginCode(ev, email))
}

async function onLoginCode(ev, email) {
  ev.preventDefault()
  const btn = document.getElementById("code-btn")
  const formEl = document.getElementById("code-form")
  ...
  const r = await send({
    type: "loginVerify",
    email,
    code: document.getElementById("lg-code").value,
    label: `Chrome · ${navigator.platform || "?"}`,
  })
  ...
  if (!r || !r.ok) {
    ...
    btn.style.width = ""
    btn.disabled = false
    errEl.textContent = (r && r.error) || "Código incorreto. Tente de novo."
    errEl.hidden = false
    formEl.classList.remove("shake")
    void formEl.offsetWidth
    formEl.classList.add("shake")
