  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <Link href="/configuracoes/catalogo" className="inline-flex items-center gap-1.5 h-9 px-3 text-xs font-semibold text-slate-600 border border-slate-200 rounded-lg bg-white hover:bg-slate-50 transition-colors">
...
function TableRow({ table }: { table: PriceTableSummary }) {
  ...
  const open = () => router.push(`/configuracoes/catalogo/tabelas/${table.id}`)
...
function NewTableDialog({ onClose }: { onClose: () => void }) {
  ...
  function create() {
    ...
    startTransition(async () => {
      ...
      onClose()
      router.push(`/configuracoes/catalogo/tabelas/${r.tableId}`)
    })
  }
