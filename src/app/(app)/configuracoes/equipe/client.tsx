...
import {
  ...
  Copy, Check, X, Loader2, MessageCircle, Mail,
} from "lucide-react"
  ...
  const memberColumns: Column<TeamMember>[] = [
    {
      ...
      cell: (m) => (
        <div className="flex items-center gap-3 min-w-0">
          <UserAvatar userId={m.user_id} name={m.full_name ?? m.email} size={36} />
    ...
    {
      id: "department",
      header: "Depto · Unidade",
      width: "180px",
      cell: (m) => (m.department || m.unit) ? (
        <div className="flex flex-wrap items-center gap-1">
          ...
          {m.unit && (
            <span
              className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-md"
              style={{ backgroundColor: m.unit.color + "20", color: m.unit.color }}
            >
              {m.unit.name}
            </span>
          )}
        ...
        <SectionCard title="Desativados" flush>
          <div className="divide-y divide-slate-100">
            {inactiveMembers.map((m) => (
              ...
              >
                <UserAvatar userId={m.user_id} name={m.full_name ?? m.email} size={32} className="opacity-50" />
