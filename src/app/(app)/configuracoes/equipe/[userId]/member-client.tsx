...
export function MemberProfileClient({ member, departments, numbers, units = [], currentUserId, currentUserRole, hasInventory = false, hasCrm = false, hasContacts = false, hasMarketing = false, hasCatalog = false }: {
  ...
  const [unitId, setUnitId]           = useState<string>(member.unit_id ?? "")
          ...
          <div className="p-6">
            ...
            {tab === "acesso" && (
              <Section title="Função & Visibilidade" sub="O que ela pode fazer e quais conversas enxerga.">
                <div className="max-w-lg space-y-5">
                  ...
                  {units.length > 0 && (
                    <Field label="Unidade" hint="Etiqueta de venda: os negócios que a pessoa abrir nascem nesta unidade. Só pra medir — não muda o que ela vê.">
                      <SimpleSelect value={unitId} onChange={setUnitId} disabled={!canEditOther}
                        options={[{ value: "", label: "— Sem unidade —" }, ...units.map((u) => ({ value: u.id, label: u.name }))]} />
                    </Field>
                  )}
            ...
            {tab === "conta" && (
              <Section title="Conta" sub="Situação da conta desta pessoa.">
                <div className="mb-6">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Dispositivos</p>
