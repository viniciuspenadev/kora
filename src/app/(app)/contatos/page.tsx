...
export default async function ContatosPage() {
  ...
  return (
    <PageShell
      variant="list"
      title="Contatos"
      description={`${total} ${total === 1 ? "contato" : "contatos"} · ${withTags} com tags`}
      actions={
        <>
          {["owner", "admin"].includes(session.user.role) && (
            <Link href="/contatos/importar" className="inline-flex items-center gap-1.5 h-9 px-4 text-xs font-semibold bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-lg transition-colors shrink-0">
