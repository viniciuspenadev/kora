...
import {
  ...
  X, AlertTriangle, Hash, Star, ChevronRight, ChevronDown, Radio, UserRound,
} from "lucide-react"
import { SectionCard } from "@/components/ui/section-card"
import { lifecycleMeta, sourceMeta } from "@/lib/lifecycle"
import { SourceLogo } from "@/components/chat/source-logo"
import { startQuoteFirst, startQuoteFirstForCompany } from "@/lib/actions/deals"
...
export function ClienteRecord({ record, appointments, activity, canEditIdentity, customFields, channels, priceTables = [], agents = [], canSetOwner = false, company = null }: { record: ContactRecord; appointments: Appt[] | null; activity: ActivityItem[]; canEditIdentity: boolean; customFields: ContactFieldDef[]; channels: ContactChannelRow[]; priceTables?: { id: string; name: string; is_default: boolean }[]; agents?: { id: string; name: string }[]; canSetOwner?: boolean; company?: { id: string; name: string } | null }) {
  ...
  // "Novo negócio" = expander quote-first (parte do CONTATO, não da conversa) — abre e cai
  // no detalhe do negócio, onde valor/itens/etapa são feitos. Sem trava de 1-aberto.
  const [menuOpen, setMenuOpen] = useState(false)
  const [dealPending, startDeal] = useTransition()
  const [dealError, setDealError] = useState<string | null>(null)
  const [busy, setBusy] = useState<null | "contact" | "company">(null)
  const canNewDeal = record.crmEnabled && record.pipelines.length > 0

  function openForContact() {
    setDealError(null); setBusy("contact")
    startDeal(async () => {
      const r = await startQuoteFirst(contact.id)
      if ("error" in r) { setDealError(r.error); setBusy(null); return }
      router.push(`/negocios/${r.dealId}`)
    })
  }
  function openForCompany() {
    if (!company) return
    setDealError(null); setBusy("company")
    startDeal(async () => {
      const r = await startQuoteFirstForCompany(company.id, contact.id)
      if ("error" in r) { setDealError(r.error); setBusy(null); return }
      router.push(`/negocios/${r.dealId}`)
    })
  }

  // Dono da conta (carteira). "Sem dono" aplica direto; escolher um dono abre o
  // fluxo deliberado (Transferir cliente — F3: prévia de impacto + pergunta de negócios).
  const [ownerId, setOwnerId] = useState(record.owner?.id ?? "")
  const [ownerPending, startOwner] = useTransition()
  const [xfer, setXfer] = useState<string | null>(null)   // novo dono pendente (modal aberto)
  const openDeals   = record.deals.filter((d) => d.status === "open").length
  const closedDeals = record.deals.filter((d) => d.status === "won" || d.status === "lost").length

  function onOwnerChange(v: string) {
    if (!v) {                                    // remover dono → direto, sem modal
      setOwnerId("")
      startOwner(async () => {
        const r = await setContactOwner(contact.id, null)
        if (r.error) { setOwnerId(record.owner?.id ?? ""); alert(r.error) }
        else router.refresh()
      })
      return
    }
    if (v === (record.owner?.id ?? "")) return   // sem mudança
    setXfer(v)                                   // abre o modal de transferência deliberada
  }

  function confirmTransfer(moveOpenDeals: boolean) {
    const newOwner = xfer
    if (!newOwner) return
    startOwner(async () => {
      const r = await transferClient(contact.id, newOwner, { moveOpenDeals })
      if ("error" in r) { alert(r.error); setXfer(null); return }
      setOwnerId(newOwner); setXfer(null); router.refresh()
    })
  }
  ...
  return (
    <div className="min-h-full bg-canvas">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {record.crmEnabled && (canSetOwner ? (
                <span className="inline-flex items-center gap-1.5">
                  ...
                  <SimpleSelect value={ownerId} onChange={onOwnerChange} className="h-7 text-xs"
            ...
            {canNewDeal && (
              <div className="relative">
                <button type="button" onClick={() => setMenuOpen((v) => !v)} aria-expanded={menuOpen}
                  className="inline-flex items-center gap-1.5 h-9 px-3 text-xs font-semibold border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-lg transition-colors">
                ...
                {menuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-2 z-30 w-72 rounded-xl border border-slate-200 bg-white shadow-lg p-1.5">
                      <p className="px-2 pt-1 pb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Abrir negócio</p>
                      <button type="button" onClick={openForContact} disabled={dealPending}
                        className="flex w-full items-center gap-2.5 rounded-lg px-2 py-2 text-left hover:bg-slate-50 transition-colors disabled:opacity-50">
                        <span className="size-7 rounded-lg bg-slate-100 text-slate-500 grid place-items-center shrink-0">
                          {busy === "contact" ? <Loader2 className="size-3.5 animate-spin" /> : <UserRound className="size-3.5" />}
                        </span>
                        <span className="min-w-0 flex-1">
                          <span className="block text-xs font-semibold text-slate-800 truncate">Para {name}</span>
                          <span className="block text-[11px] text-slate-400">Negócio da pessoa</span>
                        </span>
                      </button>
                      {company && (
                        <button type="button" onClick={openForCompany} disabled={dealPending}
                          className="flex w-full items-center gap-2.5 rounded-lg px-2 py-2 text-left hover:bg-primary-50 transition-colors disabled:opacity-50">
                          <span className="size-7 rounded-lg bg-white border border-primary-100 text-primary-600 grid place-items-center shrink-0">
                            {busy === "company" ? <Loader2 className="size-3.5 animate-spin" /> : <Building2 className="size-3.5" />}
                          </span>
                          <span className="min-w-0 flex-1">
                            <span className="block text-xs font-semibold text-slate-800 truncate">Para {company.name}</span>
                            <span className="block text-[11px] text-slate-400">Negócio da empresa</span>
                          </span>
                        </button>
                      )}
                      {dealError && <p className="mt-1 mx-1 mb-0.5 text-[11px] text-red-700 bg-red-50 border border-red-100 rounded-md px-2 py-1.5">{dealError}</p>}
                    </div>
                  </>
                )}
              </div>
      ...
      <TransferClientModal
        ...
        contactName={name}
        currentOwnerName={record.owner?.name ?? null}
        newOwnerName={agents.find((a) => a.id === xfer)?.name ?? ""}
        openDeals={openDeals}
