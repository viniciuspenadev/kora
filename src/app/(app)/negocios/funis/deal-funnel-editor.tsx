...
import {
  ...
  Trophy, XCircle, EyeOff,
} from "lucide-react"
import {
  updateDealPipeline, setDefaultDealPipeline, archiveDealPipeline,
...
export function DealFunnelEditor({ pipeline, stages: initial }: { pipeline: DealEditorPipeline; stages: DealEditorStage[] }) {
  ...
  function saveName() {
    setEditName(false)
    if (name.trim() && name.trim() !== pipeline.name) start(async () => { try { await updateDealPipeline(pipeline.id, { name: name.trim() }) } catch (e) { alert((e as Error).message) } })
  }
  ...
  async function archive() {
    if (!(await confirm({ title: `Arquivar o funil "${pipeline.name}"?`, body: "Ele sai do board de negócios. O histórico é preservado.", confirmLabel: "Arquivar" }))) return
    start(async () => { try { await archiveDealPipeline(pipeline.id); router.push("/negocios/funis") } catch (e) { alert((e as Error).message) } })
  }
  ...
  function onDrop(targetId: string) {
    ...
    start(async () => { try { await reorderDealStages(pipeline.id, next.map((s) => s.id)) } catch (e) { alert((e as Error).message) } })
  }

  async function removeStage(s: DealEditorStage) {
    if (!(await confirm({ title: `Excluir a etapa "${s.name}"?`, confirmLabel: "Excluir" }))) return
    start(async () => {
      try { await deleteDealStage(s.id); setStages((p) => p.filter((x) => x.id !== s.id)) }
  ...
  return (
    <div className="max-w-3xl">
      <div className="pb-3 text-xs flex items-center gap-1.5 text-slate-400">
        <Link href="/negocios" className="hover:text-slate-600">Negócios</Link>
        <ChevronRight className="size-3 text-slate-300" />
        <Link href="/negocios/funis" className="hover:text-slate-600">Funis</Link>
        <ChevronRight className="size-3 text-slate-300" />
            ...
            <div key={stage.id} draggable={editing !== stage.id}
              ...
              {editing === stage.id ? (
                ...
                <div className="flex items-center gap-2.5 px-3 py-2.5">
                  ...
                  {stage.dealCount > 0 && (
                    <span className="text-[10px] text-slate-400 bg-white border border-slate-200 rounded-full px-1.5 py-0.5 shrink-0 tabular-nums" title={`${stage.dealCount} negócio(s)`}>
                      {stage.dealCount} negócio{stage.dealCount === 1 ? "" : "s"}
                    </span>
                  )}
...
function StageEditRow({ stage, onClose, onSaved }: { stage: DealEditorStage; onClose: () => void; onSaved: (u: Partial<DealEditorStage>) => void }) {
  ...
  function save() {
    start(async () => {
      try {
        ...
        await updateDealStage(stage.id, data)
        onSaved(data)
      } catch (e) { alert((e as Error).message) }
    })
  }
...
function NewStageForm({ pipelineId, onClose }: { pipelineId: string; onClose: () => void }) {
  ...
  function submit() {
    if (!name.trim()) return
    start(async () => {
      try { await createDealStage(pipelineId, { name: name.trim(), color, probability_pct: 50 }); router.refresh(); onClose() }
      catch (e) { alert((e as Error).message) }
    })
  }
