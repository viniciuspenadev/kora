...
export default async function AdminDashboardPage() {
  ...
  //
  // ⚠️ DEGRADA, NÃO DERRUBA (F1, 11/08). `computeBillingSummary` passou a LANÇAR quando
  //    qualquer das 4 leituras falha — certo lá dentro, porque antes o MRR saía zerado ou
  //    parcial sem nenhum sinal. Mas esta é a HOME do painel: deixar um KPI entre vários
  //    derrubar a tela inteira do operador por um blip de segundos é desproporcional, e
  //    contraria o princípio do projeto de que **execução para, leitura degrada**.
  // 🔑 O KPI vira "—" (indisponível), que é honesto. O que NÃO se faz aqui é cair pra
  //    zero: "R$ 0,00" é a mentira silenciosa que originou a correção.
  // ⚠️ Em `/admin/financeiro` a escolha é a OPOSTA de propósito: lá a página existe só
  //    para o número: mostrá-lo errado é pior que mostrar tela de erro.
  let billing: Awaited<ReturnType<typeof computeBillingSummary>> | null = null
  try {
    billing = await computeBillingSummary()
  } catch (e) {
    console.error(JSON.stringify({
      src: "admin-home", kind: "mrr-indisponivel-kpi-degradado", msg: (e as Error).message,
    }))
  }
    ...
    <div className="min-h-full">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          <Kpi
            label="MRR"
            value={billing ? BRL(billing.mrr_cents) : "—"}
            subtitle={billing
              ? `${billing.billed} ${billing.billed === 1 ? "tenant cobrado" : "tenants cobrados"}`
              : "indisponível agora"}
            icon={Wallet}
            tone="primary"
          />
