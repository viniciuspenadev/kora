...
export default async function TenantsPage({ searchParams }: {
  // `?plano=<id>` vem da tela de Planos: a contagem "N clientes neste plano" virou porta.
  ...
  const [{ data: tenants }, { data: billing }, { data: members }, { data: channels }, { data: sessions }] =
    await Promise.all([
      supabaseAdmin
        .from("tenants")
        .select("id, name, slug, plan, plan_id, active, lifecycle_state, trial_ends_at, created_at, plans ( name )")
  ...
  const rows: TenantRow[] = (tenants ?? []).map((t) => {
    ...
    return {
      ...
      slug:               t.slug,
      plan:               t.plan,
      plan_id:            (t as { plan_id: string | null }).plan_id ?? null,
      plan_name:          planName,
      active:             t.active,
    ...
    <div className="min-h-full">
      ...
      <div className="px-6 py-6">
        <TenantsListClient
          ...
          // Nome resolvido AQUI, das próprias linhas: evita uma consulta só pra escrever
          // um rótulo, e se o id não casar com ninguém a tira nem promete um nome.
          planoNome={planoId ? (rows.find((r) => r.plan_id === planoId)?.plan_name ?? null) : null}
        />
      </div>
    </div>
  )
