...
export interface TenantRow {
  ...
  slug:       string
  plan:       string
  plan_id:    string | null   // id do plano atribuído — é por ele que a tela de Planos filtra
...
export function TenantsListClient({ rows, planoId, planoNome }: {
  rows: TenantRow[]
  /**
   * Chegou de `/admin/planos` por `?plano=<id>` — a lista abre já recortada nesse plano.
   *
   * 🔑 A contagem "3 clientes neste plano" era um número morto. Numa operação de venda
   *    assistida, a pergunta seguinte a todo preço é *"quem está nele?"* — e sem destino
   *    a resposta era abrir a lista e conferir cliente por cliente.
   * ⚠️ Filtra por **id**, não por nome: duplicar um plano gera nomes parecidos ("PLANO II
   *    (cópia)"), e filtrar por texto juntaria os dois.
   */
  planoId?:   string | null
  ...
  // O recorte por plano é dispensável na tela — some no primeiro clique e não volta.
  const [plano, setPlano] = useState<string | null>(planoId ?? null)
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      if (plano && r.plan_id !== plano) return false
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard flush>
        {/* Recorte que veio da tela de Planos — dito em português e com saída visível.
            Filtro herdado de outra tela sem aviso é a pessoa achando que sumiu cliente. */}
        {plano && (
          <div className="px-5 py-2.5 border-b border-slate-100 bg-primary-50/40 flex items-center gap-2">
            <span className="text-xs text-slate-600">
              Mostrando só quem está no plano{" "}
              <strong className="font-semibold text-slate-900">{planoNome ?? "selecionado"}</strong>
              <span className="text-slate-400"> · {filtered.length} {filtered.length === 1 ? "cliente" : "clientes"}</span>
            </span>
            <button type="button" onClick={() => setPlano(null)}
