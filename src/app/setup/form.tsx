...
export function SetupForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      // Auto-login pós-criação: mesmo caminho de 2 etapas do signin (device
      // trust F2) — o provider de senha não existe mais no NextAuth.
      // Confiança foi semeada no registerSuperAdmin → vem ticket direto. Se a
      // semeadura falhou (raro), vem "challenge" (código já foi pro e-mail);
      // esta tela não coleta código → manda pro signin (que coleta) com o e-mail
      // preenchido, em vez de mostrar "falhou".
      const begin = await beginLogin(email, password)
      if (begin.ok && "challenge" in begin) {
        router.push(`/auth/signin?email=${encodeURIComponent(email)}`)
        return
      }
      const signInResult = begin.ok && "ticket" in begin
        ? await signIn("ticket", { ticket: begin.ticket, redirect: false })
        : { error: "begin" }
