...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: msg, error: msgErr } = await supabaseAdmin
    ...
    .select(`
      ...
      chat_conversations ( assigned_to, participants, department_id )
    `)
    .eq("id", msgId)
  ...
  const conv = msg.chat_conversations as unknown as { assigned_to: string | null; participants: string[] | null; department_id: string | null } | null
  if (!conv) {
