...
async function handleMessageUpsert(
  ...
  for (const msg of messages) {
    ...
    const contact      = await findOrCreateContact(tenantId, jid, phone, pushName, instance)
 ...
 *
 * `_isNew`:
 *   - true → conversa nasceu agora (system msgs, etc)
 *   - false → conv pré-existia (apenas updates incrementais)
 *     (Reopen NÃO conta como new — retorno de
...
async function findOrCreateConversation(
  ...
  // shape `_isNew`/`_reopened` que o resto deste webhook espera.
  // Nota: dropa o antigo "criar etapa Triagem on-the-fly" — o helper resolve a
  // etapa (triagem → fallback 1ª por posição) sem fabricar stage num webhook.
