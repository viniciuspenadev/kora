...
export async function GET(req: NextRequest) {
  ...
  const saida = await executarJob({ job: "trial-housekeeping" }, async () => {
    ...
    // ⚠️ REDE ACRESCENTADA NA F1 (11/08). `runTrialHousekeeping` passou a LANÇAR quando a
    //    leitura de trials vencidos falha — correto lá dentro (antes reportava
    //    `suspended: 0` como sucesso). Mas sem esta rede a exceção subia e derrubava
    //    `reconcileAsaas` e `reconcileStorage` junto, violando a invariante declarada duas
    //    linhas acima. Mesmo tratamento que a reconciliação já recebia: registra, segue, e
    //    o `status` conta a verdade em vez de o job morrer no primeiro tropeço.
    let result: Awaited<ReturnType<typeof runTrialHousekeeping>> | null = null
    let erroHousekeeping: string | null = null
    try {
      result = await runTrialHousekeeping()
    } catch (e) {
      erroHousekeeping = (e as Error).message
      console.error("[cron/trial-housekeeping]", erroHousekeeping)
    }
    ...
    return {
      ...
      meta: {
        ...
        storage,
        billing,
        ...(erroHousekeeping ? { erroHousekeeping } : {}),
      },
