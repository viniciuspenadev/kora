...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    //      • não somava `unread_count` → a linha subia no topo sem bolinha, e quem varre o
    //        inbox pela bolinha passava direto;
    //      • não reabria conversa `resolved` → o cliente falava para um atendimento
    //        concluído, e ninguém era avisado.
    //    Também não era escopado por tenant (`.eq("id")` sozinho).
    // ⚠️ Aqui a âncora É carimbada, e isso é correto: diferente do `/api/site/lead`
    //    (formulário, que pode cair no fio de OUTRO canal), aqui é mensagem de verdade
    //    do visitante NO PRÓPRIO canal do fio. É o caso legítimo de rearmar o relógio.
    ...
    after(async () => {
      try {
        ...
        if (!(await siteAiWithinBudget(tenant.id))) {
          await routeToHumanDefault(tenant.id, convId, "site_budget_exhausted")
