...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    //    (contato + conversa): zero LLM, zero envio, zero mídia. Gatear por
    ...
    if (dedup.found !== "none") {
      ...
      await bumpConversationInbound({
        ...
        preview:        newPreview,
        // 🔴 NÃO mexe na âncora da janela. O dedup acima é SEM escopo de canal (de
        //    propósito, ver comentário na chamada), então este fio pode ser o WhatsApp
        //    OFICIAL do contato — e `last_inbound_at` é a âncora da janela de 24h da
        //    Meta. Carimbar aqui reabriria a janela por um FORMULÁRIO: o composer
        //    liberaria texto livre, a Meta recusaria com 131047, e a mensagem ficaria
        //    "enviada" sem nunca chegar. Um formulário de site não é mensagem do
        //    cliente no canal que governa esta janela.
        // ⚠️ O webchat (`/api/site/message`) é o oposto e carimba: lá é mensagem de
        //    verdade do visitante, no próprio canal do fio.
        touchWindow:    false,
        metadata: {
          site_lead: {
            page_url:  body.url ?? null,
      ...
      //    Roteamento por setor/pessoa agora é o nó Transferir do Kora Studio.
    }
    ...
    await supabaseAdmin.from("chat_messages").insert([
      ...
      },
      {
        ...
        is_private_note: false,
        metadata:        {
          // Marcador genérico: esta mensagem NÃO abre a janela do canal do fio (é um
          // formulário de site, que pode estar aterrissando num fio de WhatsApp).
          // O composer (`chat-panel.tsx`) e qualquer futuro leitor respeitam isto.
