...
async function reconciliar(): Promise<ReconcileResult> {
  ...
  for (const t of (orfas ?? []) as { id: string; asaas_subscription_id: string; plan_id: string | null }[]) {
    ...
    if (existente) {
      ...
      try {
        ...
        //
        // 🔴 O `catch` ERA MUDO (F1 da refundação, 11/08). O log de `completar_a_mao`
        //    abaixo dizia QUE `billing_day` ficou faltando, e nunca POR QUÊ — e as causas
        //    pedem ações opostas: assinatura que não existe mais no gateway é um caso; token
        //    inválido ou Asaas fora do ar é outro, e esse volta sozinho na próxima rodada.
        //    Sem a causa, quem for completar à mão começa adivinhando.
        console.error(JSON.stringify({ src: "reconcile", kind: "ciclo-do-gateway-nao-lido",
  ...
  for (const t of (travados ?? []) as { id: string; asaas_subscription_id: string; asaas_customer_id: string | null }[]) {
    ...
    try {
      ...
      //
      // 🔴 E FALHA DE CONSULTA NÃO É "NUNCA FOI VISTO" (F1 da refundação, 11/08). O `error`
      //    era descartado: um blip devolvia `data: null`, a varredura concluía que o
      //    pagamento nunca tinha sido processado e **reinjetava o evento**. Hoje a PK
      //    `reconcile_<payment>` contém o dano (o 2º INSERT bate em 23505), mas isso é
      //    sorte de chave, não decisão: basta o evento anterior ter entrado por outro id —
      //    ou qualquer efeito futuro de `liberar` deixar de ser idempotente — pra virar
      //    crédito em dobro. Dedup que erra pro lado do "processa de novo" não é dedup.
      // ⚠️ Não sabemos ⇒ não injetamos. O tenant continua na varredura da próxima rodada
      //    (15 min); o erro entra na conta de `erros` do resultado, que é o canal do laço.
      const { data: jaVisto, error: jaVistoErr } = await supabaseAdmin
      ...
      if (jaVistoErr) {
        out.erros++
        console.error(JSON.stringify({ src: "reconcile", kind: "dedup-de-pagamento-indisponivel",
          tenant: t.id, payment: pago.id, msg: jaVistoErr.message }))
        continue
      }
