...
/** Corpo do template (var keys) do cache `wa_templates` + guarda o status (fail-closed). */
async function loadTemplateBody(t: string, name: string, language: string): Promise<{ varKeys: string[]; approved: boolean } | null> {
...
async function sentLast24h(t: string, instanceId: string): Promise<number> {
  ...
  const { data: camps } = await supabaseAdmin.from("campaigns").select("id").eq("tenant_id", t).eq("instance_id", instanceId)
  const ids = ((camps ?? []) as { id: string }[]).map((c) => c.id)
  if (!ids.length) return 0
  const { count } = await supabaseAdmin.from("campaign_recipients")
...
export async function handleCampaignInbound(t: string, contactId: string, text: string): Promise<void> {
  try {
    ...
    if (OPT_OUT_WORDS.has(first) || OPT_OUT_WORDS.has(norm)) {
    ...
    const { data: recent } = await supabaseAdmin.from("campaign_recipients")
      .select("id").eq("tenant_id", t).eq("contact_id", contactId)
      .in("status", ["sent", "delivered", "read"]).order("sent_at", { ascending: false, nullsFirst: false }).limit(1).maybeSingle()
    if (recent) {
      await supabaseAdmin.from("campaign_recipients")
        .update({ status: "replied", replied_at: now }).eq("id", (recent as { id: string }).id).eq("tenant_id", t)
    }
  } catch (e) {
 ...
 */
export async function campaignFlowToTrigger(t: string, contactId: string): Promise<{ flowId: string; campaignId: string; recipientId: string } | null> {
  ...
  const { data: camp } = await supabaseAdmin.from("campaigns")
    .select("flow_id, status").eq("id", r.campaign_id).eq("tenant_id", t).maybeSingle()
  const c = camp as { flow_id: string | null; status: string } | null
  if (!c?.flow_id) return null
  return { flowId: c.flow_id, campaignId: r.campaign_id, recipientId: r.id }
}
