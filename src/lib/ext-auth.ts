...
// ═══════════════════════════════════════════════════════════════
// Doc: docs/browser-extension-design.md §3. Pipeline fail-closed em TODA request:
// Bearer → hash lookup (não-revogado) → tenant vivo + companion_enabled →
// membership ativa + companion_access → ViewerScope (mesmo mapeador da sessão).
// O token em claro NUNCA é armazenado — só sha256. Revogar = revoked_at.

const IS_PROD = process.env.NODE_ENV === "production"

// Mesmo hash dummy do auth.ts — iguala o tempo de resposta quando o email não
// existe (anti enumeração por timing).
const DUMMY_HASH = "$2b$10$xr7Cmkh6uOtBxebNKDHUBO/XnyR/m9z.2mO6moQukhXusLjLm2XVm"

// Estados de lifecycle que negam acesso (espelho do auth.ts).
...
export async function createDeviceToken(input: {
  ...
  if (!valid) throw new ExtError(401, "E-mail ou senha inválidos.")

  // Membership ativa + tenant vivo (mesmos gates do login do app).
  ...
  if (accessible.length > 0) {
    ...
    if (!tenErr && tens) {
      ...
      if (accessible.length > 0) {
        ...
        return {
          token,
          user:   { id: profile.id, name: profile.full_name ?? null, email: profile.email },
