...
//      da transferência MANUAL (transferConversation): fila do setor limpa a atribuição;
//      agent atribui a conversa; owner consulta o contato; pool limpa o destino.
//      Nenhuma transferência altera chat_contacts.owner_id.
  ...
  execute: async (ctx, args) => {
    ...
    if (args.target === "department") {
      ...
      const { data: member } = await supabaseAdmin
        .from("tenant_users")
        .select("user_id, department_id, role, view_all, instance_ids")
        .eq("tenant_id", tenantId).eq("user_id", args.agentId).eq("active", true)
        .maybeSingle()
      if (!member || !memberAttendsNumber(member, convRow.instance_id)) return fail("O atendente configurado não está ativo ou não atende este número.")
      ...
      const destination = await responsibleDestination(tenantId, convRow.contact_id, convRow.instance_id)
      assignTo = destination?.agentId ?? null
      deptId = destination?.departmentId ?? null
      label = destination ? "o responsável pelo cliente" : "a fila geral (cliente sem responsável elegível)"
    } else { // pool
    ...
    const latest = ctx.dryRun
      ? (await supabaseAdmin.from("chat_conversations").select("*").eq("tenant_id", tenantId).eq("id", conversationId).maybeSingle()).data
      : await assertStudioControl(ctx)
    if (!latest) return { ok: false, error: "Conversa indisponível." }
    if (latest.status !== "open" || latest.assigned_to !== prevOwner
        || JSON.stringify(latest.metadata?.ai_routed ?? null) !== JSON.stringify(convRow.metadata?.ai_routed ?? null)) {
      return { ok: false, error: "A conversa mudou durante a transferência." }
    }
    const nextMeta: Record<string, unknown> = { ...latest.metadata,
      ai_routed: { department_id: deptId, department_name: deptName, via: "studio_transfer", at: new Date().toISOString() } }
    delete nextMeta.reopen_owner
    delete nextMeta.ai_pinned_flow
    delete nextMeta.studio_entry
    const { data: changed, error: writeError } = await supabaseAdmin
