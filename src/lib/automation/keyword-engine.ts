...
export async function evaluateKeywordTriggers(input: EvaluateInput): Promise<boolean> {
  ...
  // A configuração pode continuar gravada após downgrade; o motor precisa consultar a
  // licença a cada execução. Erro da RPC vira false em `hasModule` (fail-closed).
  if (!(await hasModule(tenantId, "keyword_triggers"))) return false
