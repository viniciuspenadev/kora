...
export async function dispatchAutomations(input: DispatchInput): Promise<void> {
  ...
  // Não confiar apenas na tela/configuração: um downgrade preserva os textos para uma
  // futura reativação, mas não pode continuar executando o recurso pago.
  const [businessHoursOn, welcomeOn] = await Promise.all([
    hasModule(tenantId, "business_hours"),
    hasModule(tenantId, "welcome_message"),
  ])
  if (!businessHoursOn && !welcomeOn) return
  ...
  if (businessHoursOn && cfg.business_hours_enabled && cfg.business_hours_message) {
  ...
  // ── 2. Welcome ──────────────────────────────────────────────
  if (welcomeOn && cfg.welcome_enabled && cfg.welcome_message) {
