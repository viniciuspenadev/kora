...
describe("o relógio da carência", () => {
  it("dentro do prazo NÃO passou", () => {
    expect(passouDaCarencia(haDias(3), 7, PADRAO, AGORA)).toBe(false)
  })

  it("no dia exato JÁ passou (a carência é de N dias, não N+1)", () => {
    expect(passouDaCarencia(haDias(7), 7, PADRAO, AGORA)).toBe(true)
  })

  it("carência 0 corta junto com o degrau 2 — é valor válido, não 'sem valor'", () => {
    expect(passouDaCarencia(haDias(0), 0, PADRAO, AGORA)).toBe(true)
  })

  it("carência nula cai no padrão do sistema, nunca em zero", () => {
    expect(passouDaCarencia(haDias(PAST_DUE_GRACE_DAYS - 1), null, PADRAO, AGORA)).toBe(false)
    expect(passouDaCarencia(haDias(PAST_DUE_GRACE_DAYS), null, PADRAO, AGORA)).toBe(true)
  })
  ...
  it("sem carimbo trata como PASSADO (senão carimbo faltando = carência infinita)", () => {
    expect(passouDaCarencia(null, 7, PADRAO, AGORA)).toBe(true)
    expect(passouDaCarencia("data-podre", 7, PADRAO, AGORA)).toBe(true)
  })
})
...
describe("de onde vem o número da carência", () => {
  it("o valor do TENANT vence o global — é pra isso que a coluna existe", () => {
    ...
    expect(passouDaCarencia(haDias(2), 1, 30, AGORA)).toBe(true)
    // Global apertado (0), tenant frouxo (10): o tenant manda de novo.
    expect(passouDaCarencia(haDias(2), 10, 0, AGORA)).toBe(false)
  })

  it("tenant NULO cai no global (e não na constante)", () => {
    expect(passouDaCarencia(haDias(5), null, 10, AGORA)).toBe(false)
    expect(passouDaCarencia(haDias(10), null, 10, AGORA)).toBe(true)
  })
  ...
  it("tenant com ZERO é valor válido, não 'sem valor' — corta na hora", () => {
    ...
    expect(passouDaCarencia(haDias(0), 0, 30, AGORA)).toBe(true)
  })

  it("global ZERO também é válido — não escorrega pra constante", () => {
    expect(passouDaCarencia(haDias(0), null, 0, AGORA)).toBe(true)
  })
  ...
  it("global corrompido cai na constante, nunca em carência infinita", () => {
    expect(passouDaCarencia(haDias(90), null, NaN, AGORA)).toBe(true)
    expect(passouDaCarencia(haDias(90), null, -5, AGORA)).toBe(true)
    expect(passouDaCarencia(haDias(90), null, Infinity, AGORA)).toBe(true)
  })
...
describe("o motivo do paywall", () => {
  it("teste vencido é paywall na hora — não há carência pra quem nunca pagou", () => {
    expect(motivoDoPaywall("trial_ended", "active", null, null, PADRAO, AGORA)).toBe("trial_ended")
  })

  it("atraso DENTRO da carência ainda não é paywall (é o degrau 2)", () => {
    expect(motivoDoPaywall("active", "past_due", haDias(2), 7, PADRAO, AGORA)).toBeNull()
  })

  it("atraso ALÉM da carência vira paywall", () => {
    expect(motivoDoPaywall("active", "past_due", haDias(8), 7, PADRAO, AGORA)).toBe("past_due")
  })
  ...
  it("assinatura em dia nunca é paywall, mesmo com carimbo residual", () => {
    ...
    expect(motivoDoPaywall("active", "active", haDias(90), 7, PADRAO, AGORA)).toBeNull()
  })
  ...
  it("cancelada É paywall — mas só DEPOIS do ciclo pago", () => {
    ...
    expect(motivoDoPaywall("active", "active", null, null, PADRAO, AGORA)).toBeNull()
    // Passada a data, a varredura carimba `canceled` ⇒ produto fecha.
    expect(motivoDoPaywall("active", "canceled", null, null, PADRAO, AGORA)).toBe("canceled")
  })
  ...
  it("o booleano é derivado do motivo, nunca uma segunda regra", () => {
    for (const sub of ["active", "past_due", "canceled"]) {
      for (const since of [null, haDias(2), haDias(30)]) {
        const m = motivoDoPaywall("active", sub, since, 7, PADRAO, AGORA)
        expect(isTenantInPaywall("active", sub, since, 7, PADRAO, AGORA)).toBe(m !== null)
      }
    }
  })
})
