...
export async function getOrCreateSiteConversation(
  ...
  const aiSeed = channelDispatchesAI("site") && await tenantAiActive(tenantId)
  const { data: created, error } = await supabaseAdmin
  ...
  if (!aiSeed) await routeToHumanDefault(tenantId, created.id, "site_created_without_studio")
