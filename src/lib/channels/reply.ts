...
export async function sendChannelText(
  ...
  switch (channel) {
    ...
    case "instagram": {
      ...
      if (!(await hasModule(target.tenantId, "instagram_direct"))) {
        throw new Error("Instagram Direct não habilitado nesta conta.")
      }
...
export async function sendChannelRich(
  ...
  if (!(await hasModule(target.tenantId, "instagram_direct"))) return null
  const igsid = target.externalId
