 ...
 *    "atribuição manual" e falseia justamente a coluna que o dono usa pra avaliar o
 *    motor. Strings literais de propósito: o relatório não importa de quem as emite.
 *
 * ⚠️ Só `auto_assign_pool` é emitido hoje (o atendente PEGANDO da fila ao responder —
 *    20 eventos em prod). Os demais vêm de motores removidos em 2026-08-26 e ficam
 *    listados porque o livro é append-only: evento antigo tem que continuar contando
 *    no balde certo.
 */
const ORIGEM_MOTOR: ReadonlySet<string> = new Set([
  "auto_assign_pool",          // o atendente PEGOU da fila ao responder
  // 🗑️ Histórico: o motor de distribuição foi removido em 2026-08-26, mas eventos
  //    antigos com este motivo ainda existem na trilha e precisam continuar somando
  //    em "fila". Literal, e não import, porque a constante morreu com o motor.
  "auto_assign_distributed",
...
interface AgentBase { id: string; name: string; department: string | null }
...
async function collectRange(
  ...
  const zero = () => ({
    ...
    origem: { fila: 0, direto: 0, transferencia: 0, ia: 0 }, janelas: 0,
  })
...
export async function getAgentReport(tenantId: string, fromISO: string, toISO: string): Promise<AgentReportData> {
  ...
  const agents: AgentBase[] = ((members ?? []) as {
    ...
    return { id: m.user_id, name: p?.full_name ?? "Atendente", department: m.department_id ? (deptName.get(m.department_id) ?? null) : null }
  })
