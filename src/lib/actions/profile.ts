...
import { revokeUserTrusts } from "@/lib/auth/trust"
...
export async function changeMyPassword(
  ...
  // se a troca foi reação a comprometimento, o invasor volta pro desafio.
  await revokeUserTrusts(session.user.id)

  // ...e os tokens da extensão (requireExtViewer só checa revoked_at, nunca
  // confiança nem password_changed_at) — sem isto a extensão do invasor seguia
  // logada indefinidamente após a troca de senha. Fecha o 3º alvo da cascata.
