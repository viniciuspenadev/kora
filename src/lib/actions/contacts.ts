...
/**
 * Transferir CLIENTE (fluxo deliberado, F3) — passa a carteira + o rastro pro novo
 * dono, e OPCIONALMENTE os negócios EM ANDAMENTO. Negócios fechados (won/lost) NUNCA
 * movem (o crédito da venda é de quem fechou). NÃO toca no assigned_to das conversas
 * (atendimento é eixo separado). Auditado (LGPD). Gate Gerenciar-contatos/admin.
 */
export async function transferClient(
  contactId:  string,
  newOwnerId: string,
  opts: { moveOpenDeals?: boolean } = {},
): Promise<{ error: string } | { movedDeals: number }> {
  const scope = await getViewerScope()
  if (!scope.tenantId) return { error: "Não autenticado" }
  if (!canManageContacts(scope)) return { error: "Só quem gerencia contatos pode transferir o cliente." }
  const t = scope.tenantId

  const { data: c } = await supabaseAdmin.from("chat_contacts").select("id, owner_id")
    .eq("id", contactId).eq("tenant_id", t).maybeSingle()
  if (!c) return { error: "Contato inválido" }
  const prevOwner = (c as { owner_id: string | null }).owner_id ?? null

  const { data: m } = await supabaseAdmin.from("tenant_users").select("user_id")
    .eq("tenant_id", t).eq("user_id", newOwnerId).eq("active", true).maybeSingle()
  if (!m) return { error: "Novo dono inválido" }
  if (newOwnerId === prevOwner) return { error: "Este já é o dono do cliente." }

  // 1. Carteira: só o CONTATO (a conversa não carrega owner_id — chat = só atendimento).
  ...
  // 2. Negócios EM ANDAMENTO (opcional). Fechados nunca (histórico/crédito da venda).
  let movedDeals = 0
  if (opts.moveOpenDeals) {
    const { data: moved } = await supabaseAdmin.from("tenant_deals")
      .update({ assigned_to: newOwnerId, updated_at: new Date().toISOString() })
      .eq("tenant_id", t).eq("contact_id", contactId).eq("status", "open")
      .select("id")
    movedDeals = (moved ?? []).length
  }

  // 3. Auditoria (LGPD art. 37): quem, quando, de→para + quantos negócios moveram.
  await logAudit({
    tenantId: t, actorId: scope.userId,
    action: "contact.transfer_owner", targetType: "contact", targetId: contactId,
    before: { owner_id: prevOwner }, after: { owner_id: newOwnerId },
    metadata: { moved_open_deals: movedDeals },
  })
