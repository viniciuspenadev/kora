...
vi.mock("@/lib/providers", () => ({ getProvider: () => ({ deleteForEveryone: mocks.edit }) }))
vi.mock("@/lib/rate-limit", () => ({ rateLimit: () => ({ ok: true }) }))
import { deleteSentMessage } from "./chat-message-deletions"
...
beforeEach(() => {
  ...
  mocks.rpc.mockImplementation(async (name: string) => {
    if (name === "reserve_chat_message_deletion") return { data: { acquired: true }, error: null }
    operationStatus = "confirmed"; message = { ...message, content: null, deleted_at: new Date().toISOString() }
    return { data: { status: "confirmed" }, error: null }
  })
  mocks.from.mockImplementation((table: string) => {
    let update: Record<string, unknown> | undefined
    const finish = () => {
      if (update && table === "chat_message_deletions" && operationStatus === "pending") operationStatus = String(update.status)
...
describe("edit server orchestration", () => {
  it("scopes every table and confirms before returning the patched content", async () => {
    expect(await deleteSentMessage(input)).toMatchObject({ message: { id: id(1), content: null } })
    for (const table of ["chat_messages", "chat_conversations", "whatsapp_instances", "chat_message_deletions"])
  ...
  it("denies revoked membership", async () => {
    ...
    expect(await deleteSentMessage(input)).toHaveProperty("error"); expect(mocks.edit).not.toHaveBeenCalled()
  })
  it("denies conversation outside visibility", async () => {
    mocks.canView.mockReturnValue(false)
    expect(await deleteSentMessage(input)).toHaveProperty("error"); expect(mocks.rpc).not.toHaveBeenCalled()
  })
  ...
  it("duplicate request checks status without retrying provider", async () => {
    ...
    expect(await deleteSentMessage(input)).toMatchObject({ uncertain: true }); expect(mocks.edit).not.toHaveBeenCalled()
  })
  it("timeout remains uncertain without leaking provider error", async () => {
    ...
    const result = await deleteSentMessage(input)
  ...
  it("webhook confirmation wins over a failed HTTP response", async () => {
    mocks.edit.mockImplementation(async () => { operationStatus = "confirmed"; message.content = null; message.deleted_at = new Date().toISOString(); throw new Error("late error") })
    expect(await deleteSentMessage(input)).toMatchObject({ message: { content: null } })
