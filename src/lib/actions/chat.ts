...
export async function saveWhatsAppConfig(formData: {
  ...
  if (existing) {
    ...
    // Novo número é recurso licenciado. Atualizar/renomear/desconectar um número já
    // existente continua permitido para não aprisionar um tenant após downgrade.
    await requireModule("multi_instance")
...
export async function addQrNumber(displayName: string): Promise<{ id?: string; error?: string }> {
  ...
  try { await requireModule("multi_instance") }
  catch { return { error: "Múltiplos números não habilitados nesta conta." } }
...
export async function sendMessage(
  ...
  const conversationChannel = (conv as { channel: string | null }).channel ?? "whatsapp"
  if (!isPrivateNote && conversationChannel === "instagram") {
    await requireModule("instagram_direct")
  }

  if (!isPrivateNote && !isWhatsAppChannel(conversationChannel) && !["site", "instagram"].includes(conversationChannel)) {
    throw new Error(`Resposta no canal '${conversationChannel}' ainda não suportada`)
  }
  if (!isPrivateNote && isWhatsAppChannel(conversationChannel) && !conv.instance_id) {
  ...
  if (!isPrivateNote) await claimAfterAcceptedReply(tenantId, conversationId, session.user.id, conv.contact_id)
...
export async function sendOfficialTemplate(
  ...
  await prepareHumanReply(tenantId, conversationId, session.user.id, assignedTo, scope)
  ...
  await claimAfterAcceptedReply(tenantId, conversationId, session.user.id, conv.contact_id)
...
interface SendContext { contactId: string | null; tenantId: string; userId: string; instanceId: string; phone: string }
...
async function resolveSendContext(
  ...
  if (opts?.autoAssign) {
    const inst = conv.whatsapp_instances as unknown as { provider: string | null } | { provider: string | null }[] | null
    const provider = Array.isArray(inst) ? inst[0]?.provider : inst?.provider
    if (!isWindowOpen(conv.channel, provider ?? null, conv.last_inbound_at)) {
      return { error: "Janela de atendimento fechada — envie um template aprovado pra reabrir a conversa." }
    }
    try { await prepareHumanReply(tenantId, conversationId, session.user.id, assignedTo, scope) }
    catch (e) { return { error: (e as Error).message } }
  ...
  return { contactId: conv.contact_id, tenantId, userId: session.user.id, instanceId: sendInstanceId, phone: contact.phone_number ?? contact.bsuid ?? "" }
}

async function bumpConv(tenantId: string, conversationId: string, preview: string) {
  await supabaseAdmin.from("chat_conversations").update({
    last_message_at: new Date().toISOString(), last_message_preview: preview,
    last_message_dir: "out", flagged_pending: false,
    updated_at: new Date().toISOString(),
...
export async function sendLocationMessage(
  ...
  try {
    ...
    const { data: msg, error: insertError } = await supabaseAdmin.from("chat_messages").insert({
      conversation_id: conversationId, tenant_id: ctx.tenantId,
      sender_type: "agent", sender_id: ctx.userId,
    ...
    if (insertError || !msg) return { error: "Não foi possível salvar a mensagem para envio." }
    pendingId = msg.id
    ...
    await supabaseAdmin.from("chat_messages").update({ status: "sent", whatsapp_msg_id: result.messageId || null })
      .eq("tenant_id", ctx.tenantId).eq("id", msg.id)
    await claimAfterAcceptedReply(ctx.tenantId, conversationId, ctx.userId, ctx.contactId)
    await bumpConv(ctx.tenantId, conversationId, "📍 Localização")
    return { id: msg?.id ?? "" }
  } catch (err) {
    if (pendingId) await supabaseAdmin.from("chat_messages").update({ status: "failed" }).eq("tenant_id", ctx.tenantId).eq("id", pendingId)
    return { error: `Não consegui enviar a localização: ${(err as Error).message}` } }
}
...
export async function sendContactMessage(
  ...
  try {
    ...
    await supabaseAdmin.from("chat_messages").update({ status: "sent", whatsapp_msg_id: result.messageId || null })
      .eq("tenant_id", ctx.tenantId).eq("id", msg.id)
    await claimAfterAcceptedReply(ctx.tenantId, conversationId, ctx.userId, ctx.contactId)
    await bumpConv(ctx.tenantId, conversationId, "👤 Contato")
    return { id: msg?.id ?? "" }
  } catch (err) {
    if (pendingId) await supabaseAdmin.from("chat_messages").update({ status: "failed" }).eq("tenant_id", ctx.tenantId).eq("id", pendingId)
    return { error: `Não consegui enviar o contato: ${(err as Error).message}` } }
}
...
export async function sendStickerMessage(
  ...
  try {
    ...
    await supabaseAdmin.from("chat_messages").update({ status: "sent", whatsapp_msg_id: result.messageId || null })
      .eq("tenant_id", ctx.tenantId).eq("id", msg.id)
    await claimAfterAcceptedReply(ctx.tenantId, conversationId, ctx.userId, ctx.contactId)
    await bumpConv(ctx.tenantId, conversationId, "Figurinha")
    return { id: msg?.id ?? "" }
  } catch (err) {
    if (pendingId) await supabaseAdmin.from("chat_messages").update({ status: "failed" }).eq("tenant_id", ctx.tenantId).eq("id", pendingId)
    return { error: `Não consegui enviar a figurinha: ${(err as Error).message}` } }
}
...
export async function assignConversation(conversationId: string, agentId: string | null) {
  ...
  const now = new Date().toISOString()
  const metadata: Record<string, unknown> = { ...conv.metadata, ai_routed: { at: now, by: session.user.id, via: "manual_assign" } }
...
export async function transferConversation(
  ...
  const nextMeta: Record<string, unknown> = { ...prevMeta, ai_routed: { at: now, by: session.user.id, via: "manual_transfer" } }
  delete nextMeta.reopen_owner
  delete nextMeta.ai_pinned_flow
  delete nextMeta.studio_entry
  const { data: transferred, error: transferError } = await supabaseAdmin
    ...
    .eq("id", conversationId)
    .eq("tenant_id", tenantId).eq("updated_at", conv.updated_at).select("id")
  if (transferError || !transferred?.length) return { error: "A conversa mudou. Atualize antes de transferir." }
...
export async function searchContacts(query: string) {
  ...
  const orParts = [`custom_name.ilike.%${q}%`, `push_name.ilike.%${q}%`, `phone_number.ilike.%${q}%`, `email.ilike.%${q}%`, `company.ilike.%${q}%`]
  const digits = q.replace(/\D/g, "")
  if (digits.length >= 3) orParts.push(`doc_id.ilike.%${digits}%`)   // localizar por CPF/CNPJ digitado
...
/* ── Grupos WhatsApp: REMOVIDO em 2026-08-03 ──────────────────────────────
 * `listPendingGroups`, `decideGroup` e `fetchAndSaveGroupMetadata` saíram junto com a
 * ingestão de grupo no webhook. Sem ingestão não existe grupo pendente pra aprovar.
 * A tabela `chat_groups_whitelist` e as 9 conversas já gravadas FICAM no banco —
 * o motivo da remoção e o que reabrir exige estão no webhook (busca por "@g.us").
 * ──────────────────────────────────────────────────────────────────────── */

// ── Quick Replies ───────────────────────────────────────────

async function requireQuickReplyAdmin() {
  const session = await auth()
  if (!session?.user?.tenantId) throw new Error("Não autenticado")
  if (!["owner", "admin"].includes(session.user.role)) throw new Error("Sem permissão")
  await requireModule("quick_replies")
  return session
}

export async function createQuickReply(data: { shortcut: string; title: string; content: string }) {
  const session = await requireQuickReplyAdmin()

  const { error } = await supabaseAdmin.from("chat_quick_replies").insert({
...
export async function updateQuickReply(id: string, data: { shortcut: string; title: string; content: string }) {
  ...
  const { data: updated, error } = await supabaseAdmin
    ...
    .maybeSingle()
  if (error) throw new Error(error.message)
  if (!updated) throw new Error("Resposta rápida não encontrada")
...
export async function deleteQuickReply(id: string) {
  ...
  const { data: deleted, error } = await supabaseAdmin
    ...
    .maybeSingle()
  if (error) throw new Error(error.message)
  if (!deleted) throw new Error("Resposta rápida não encontrada")
