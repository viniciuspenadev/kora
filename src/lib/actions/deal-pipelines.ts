...
export async function updateDealStage(
  ...
  const updates = pickDefined(data, [
    "name", "color", "probability_pct", "is_won", "is_lost", "show_in_kanban",
  ] as const)
  const { error } = await supabaseAdmin.from("deal_pipeline_stages").update(updates).eq("id", id).eq("tenant_id", session.user.tenantId)
  if (error) throw new Error(error.message)
  revalidate()
}
