...
export async function updatePipeline(id: string, data: Partial<{ name: string; description: string | null; color: string }>) {
  ...
  // A assinatura TS não é fronteira de segurança: Server Actions recebem JSON
  // adulterável. A allow-list é aplicada em runtime antes de chegar ao banco.
  const updates = {
...
export async function updateStage(
  ...
  const updates = pickDefined(data, [
    "name", "color", "probability_pct", "position", "is_won", "is_lost", "show_in_kanban",
  ] as const)
