...
export async function getMyDay(input?: { scope?: "me" | "team"; horizonDays?: number }): Promise<MyDay> {
  ...
  if (crmOn) {
    ...
    items.push(...[...tasks.items,...completed.items].filter(t => t.due_at).map(t => ({
      ...
      // A fonte comercial não pode interromper os compromissos já carregados.
      // Retorna indisponibilidade explícita, nunca uma lista vazia como sucesso.
      crmUnavailable = true
      console.error("[my-day] leitura de tarefas CRM indisponível")
    }
  }
  items.sort((a, b) => new Date(a.at).getTime() - new Date(b.at).getTime())
  return { items, canSeeTeam, agendaOn, crmOn, tasksLimited, crmUnavailable }
}
