...
export interface OpenDealInput {
  ...
  companyId?:      string | null   // âncora de EMPRESA opcional (aditiva) — contato-da-conversa + empresa
}
...
export async function openDeal(input: OpenDealInput): Promise<{ id: string } | { error: string }> {
  ...
  const { data: stg } = await supabaseAdmin.from("deal_pipeline_stages")
    .select("id").eq("id", input.stageId).eq("pipeline_id", input.pipelineId).eq("tenant_id", session.user.tenantId).maybeSingle()
  if (!stg) return { error: "Etapa inválida" }

  return createDeal({
    tenantId:       session.user.tenantId,
    contactId:      conv.contact_id,
    companyId:      input.companyId ?? null,   // empresa aditiva (createDeal valida tenant/anti-IDOR)
...
export interface DealsPanel {
  ...
  /** Contato da conversa + empresa vinculada (se houver) — pro overlay "Novo negócio"
   *  ancorar na empresa e mostrar o cartão. `company=null` → negócio da pessoa. */
  contactId:    string | null
  company:      { id: string; name: string; legal_name: string | null; doc_id: string | null; archived_at: string | null } | null
}
...
export async function getDealsPanel(conversationId: string): Promise<DealsPanel> {
  const empty: DealsPanel = { enabled: false, activeDealId: null, relationship: "prospect", wonCount: 0, deals: [], pipelines: [], contactId: null, company: null }
  const session = await auth()
  ...
  // Empresa vinculada do contato (pro overlay "Novo negócio" ancorar na conta comercial).
  let company: DealsPanel["company"] = null
  if (conv.contact_id) {
    const { data: ct } = await supabaseAdmin.from("chat_contacts").select("company_id").eq("id", conv.contact_id).eq("tenant_id", tenantId).maybeSingle()
    const cid = (ct as { company_id: string | null } | null)?.company_id ?? null
    if (cid) {
      const { data: co } = await supabaseAdmin.from("tenant_companies").select("id, name, legal_name, doc_id, archived_at").eq("id", cid).eq("tenant_id", tenantId).maybeSingle()
      company = (co as DealsPanel["company"]) ?? null
    }
  }

  return { enabled: true, activeDealId: conv.active_deal_id, relationship, wonCount, deals, pipelines, contactId: conv.contact_id, company }
}
...
// Composição pura do motor central (createDealFromBoard/createDeal → compositor EXISTENTE;
// nada de snapshot/tabela/addItem próprios). Multi-negócio: sempre cria novo. docs crm-proposals.
...
/** Resolve a etapa-alvo do quote-first: se veio uma etapa explícita (ex: coluna clicada no
 *  funil), VALIDA que é do tenant, do pipeline informado e NÃO é terminal (anti-IDOR — não
 *  confiar no id vindo do client); senão cai na etapa de entrada do funil padrão. */
async function resolveTargetStage(tenantId: string, stage?: { pipelineId: string; stageId: string } | null): Promise<{ pipelineId: string; stageId: string } | null> {
  if (!stage) return defaultDealPipelineStage()
  const { data } = await supabaseAdmin.from("deal_pipeline_stages")
    .select("id, pipeline_id, is_won, is_lost")
    .eq("id", stage.stageId).eq("pipeline_id", stage.pipelineId).eq("tenant_id", tenantId).maybeSingle()
  const s = data as { id: string; pipeline_id: string; is_won: boolean; is_lost: boolean } | null
  if (!s || s.is_won || s.is_lost) return null
  return { pipelineId: s.pipeline_id, stageId: s.id }
}

/** Próximo negócio ABERTO do contato (o mais recente), excluindo `excludeDealId`. Multi-negócio:
 *  quando o em-foco FECHA, o `active_deal_id` da conversa reaponta pra este (ou null). */
async function nextOpenDealId(tenantId: string, contactId: string, excludeDealId: string): Promise<string | null> {
  const { data } = await supabaseAdmin.from("tenant_deals")
    .select("id").eq("tenant_id", tenantId).eq("contact_id", contactId).eq("status", "open").neq("id", excludeDealId)
 ...
 *  Multi-negócio (2026-07-28): SEMPRE cria um negócio novo — não anexa mais ao aberto. */
export async function startQuoteFirst(contactId: string, stage?: { pipelineId: string; stageId: string } | null): Promise<{ dealId: string } | { error: string }> {
  const session = await auth()
  if (!session?.user?.tenantId) return { error: "Não autenticado" }
  const t = session.user.tenantId
  const scope = await getViewerScope()
  if (!canOpenDeals(scope)) return { error: "Sem permissão" }
  try { await requireModule("crm") } catch { return { error: "Módulo CRM não habilitado" } }
  // Ownership/alcance do contato (mesmo gate do createDealFromBoard).
...
/** Quote-first ancorado em EMPRESA (F2) — abre negócio da empresa, com pessoa de contato
 *  OPCIONAL e aditiva (`contactId`). Reusa `createDeal` (motor central), que cria o negócio
 *  com company_id (+ contact_id se vier) numa tacada. Multi-negócio: SEMPRE cria novo (a
 *  trava "1 aberto" foi removida). Sem contato = só-empresa (legítimo). */
export async function startQuoteFirstForCompany(companyId: string, contactId?: string | null, stage?: { pipelineId: string; stageId: string } | null): Promise<{ dealId: string } | { error: string }> {
  const session = await auth()
  if (!session?.user?.tenantId) return { error: "Não autenticado" }
  const t = session.user.tenantId
  const scope = await getViewerScope()
  if (!canOpenDeals(scope)) return { error: "Sem permissão" }
  try { await requireModule("crm") } catch { return { error: "Módulo CRM não habilitado" } }
  const { data: company } = await supabaseAdmin.from("tenant_companies").select("id").eq("id", companyId).eq("tenant_id", t).maybeSingle()
  if (!company) return { error: "Empresa inválida" }
  // Contato aditivo: valida tenant + alcance por-atendente (mesmo gate do startQuoteFirst).
  if (contactId) {
    const { data: contact } = await supabaseAdmin.from("chat_contacts").select("id").eq("id", contactId).eq("tenant_id", t).maybeSingle()
    if (!contact) return { error: "Contato inválido" }
    if (!seesAllContacts(scope)) {
...
export interface DealsPageData {
  ...
  /** Unidades ATIVAS do tenant (dimensão do CRM). Vazio/1 = filtro de unidade some. */
...
export async function getDealsPage(opts?: { from?: string; to?: string }): Promise<DealsPageData | { error: string }> {
  ...
  const [{ data }, { data: pipes }, { data: members }, { data: unitRows }] = await Promise.all([
    dealsQ,
    ...
    supabaseAdmin.from("tenant_units").select("id, name, color").eq("tenant_id", t).eq("active", true).order("name"),
  ])
  ...
  const units = (unitRows ?? []) as { id: string; name: string; color: string }[]

  return { kpis, deals, pipelines, agents, allTags, units, period: { from, to } }
}

// ── Ficha do Negócio (drawer) ───────────────────────────────────

/** Pode ver/agir no negócio? Manager (admin/view_all) OU vê alguma conversa do contato. */
/** Pode GERENCIAR (write pesado: reabrir/editar) ESTE negócio? = dono OU participante
 *  do negócio, OU gestor (seesAllDeals). Distinto de canAccessDeal (que dá acesso via
 *  CONVERSA) — write de gestão exige POSSE do negócio, não só ver a conversa (fail-closed). */
function canManageDealRow(scope: ViewerScope, deal: { assigned_to: string | null }): boolean {
...
export async function canAccessDeal(tenantId: string, contactId: string | null, assignedTo: string | null = null): Promise<boolean> {
  ...
  // OU é DONO de algum negócio do contato (posse do negócio) — senão o dono sem
  // conversa leva "Sem acesso" ao agendar follow-up/tarefa.
  const { data: deals } = await supabaseAdmin
    .from("tenant_deals").select("assigned_to")
    .eq("tenant_id", tenantId).eq("contact_id", contactId)
  return ((deals ?? []) as { assigned_to: string | null }[])
    .some((d) => d.assigned_to === scope.userId)
}

// [removido] Participação no NEGÓCIO (addDealParticipant/removeDealParticipant) —
// feature descontinuada 2026-07-11 (owner). Negócio = dono único + gestores. Colaboração
// segue existindo só na CONVERSA (chat_conversations.participants). Coluna
// tenant_deals.participants fica dormante (drop pós-deploy).
...
export async function getDeal(dealId: string): Promise<DealDetail | { error: string }> {
  ...
  return {
    ...
    unit_id: (deal.unit_id as string | null) ?? null,
    unit: (deal.tenant_units as { id: string; name: string; color: string } | null) ?? null,
  }
}
...
export async function updateDeal(dealId: string, fields: { name?: string; estimatedValue?: number | null; expectedClose?: string | null; paymentMethod?: string | null; installments?: number | null; proposalExpiresAt?: string | null; priceTableId?: string | null; unitId?: string | null }, opts?: { silentCard?: boolean }): Promise<{ ok: true } | { error: string }> {
  const session = await auth()
  if (!session?.user?.tenantId) return { error: "Não autenticado" }
  try { await requireModule("crm") } catch { return { error: "Módulo CRM não habilitado" } }
  const t = session.user.tenantId
  const { data: deal } = await supabaseAdmin.from("tenant_deals")
    .select("contact_id, name, estimated_value, expected_close_date, payment_method, installments, proposal_expires_at, price_table_id, unit_id, assigned_to")
    .eq("id", dealId).eq("tenant_id", t).maybeSingle()
  if (!deal) return { error: "Negócio não encontrado" }
  const d = deal as { contact_id: string | null; name: string | null; estimated_value: number | null; expected_close_date: string | null; price_table_id: string | null; unit_id: string | null; assigned_to: string | null }
  // Editar é write de GESTÃO → exige posse do negócio (dono/participante) ou gestor,
  // não só acesso via conversa (fail-closed).
  const scope = await getViewerScope()
  if (!canManageDealRow(scope, d)) return { error: "Só o dono/participante do negócio ou um gestor pode editar." }
  ...
  if (fields.unitId !== undefined) {
    // Unidade (dimensão do CRM): exceção manual ao carimbo do vendedor. Anti-IDOR:
    // a unidade tem que ser DO tenant. "Sem unidade" (null) é opção válida.
    const cur = d.unit_id ?? null
    const next = fields.unitId || null
    if (next !== cur) {
      const nameOf = async (id: string | null): Promise<string> => {
        if (!id) return "Sem unidade"
        const { data: u } = await supabaseAdmin.from("tenant_units").select("name").eq("id", id).eq("tenant_id", t).maybeSingle()
        return (u as { name: string } | null)?.name ?? "?"
      }
      if (next) {
        const { data: u } = await supabaseAdmin.from("tenant_units").select("id").eq("id", next).eq("tenant_id", t).maybeSingle()
        if (!u) return { error: "Unidade inválida para este tenant" }
      }
      patch.unit_id = next
      changes.push({ label: "Unidade", from: await nameOf(cur), to: await nameOf(next) })
    }
  }
  ...
  if (status === "open") {
    if (conversationId) {
      await supabaseAdmin.from("chat_conversations")
        .update({ active_deal_id: dealId, updated_at: now }).eq("id", conversationId).eq("tenant_id", t)
    }
  } else if (d.contact_id) {
    const nextActive = await nextOpenDealId(t, d.contact_id, dealId)
    await supabaseAdmin.from("chat_conversations")
      .update({ active_deal_id: nextActive, updated_at: now })
      .eq("tenant_id", t).eq("contact_id", d.contact_id).eq("active_deal_id", dealId)
  }
...
export async function reopenDealById(dealId: string): Promise<{ ok: true } | { error: string }> {
  ...
  try { await requireModule("crm") } catch { return { error: "Módulo CRM não habilitado" } }
  const t = session.user.tenantId
  const { data: deal } = await supabaseAdmin.from("tenant_deals").select("contact_id, stage_id, status, assigned_to").eq("id", dealId).eq("tenant_id", t).maybeSingle()
  if (!deal) return { error: "Negócio não encontrado" }
  const dr = deal as { contact_id: string | null; stage_id: string | null; status: string; assigned_to: string | null }
  const reopenContactId = dr.contact_id
  // Reabrir é write de GESTÃO → posse do negócio ou gestor (fail-closed). GANHO exige gestor.
  const scope = await getViewerScope()
  if (!canManageDealRow(scope, dr)) return { error: "Só o dono/participante do negócio ou um gestor pode reabrir." }
  if (dr.status === "won" && !["owner", "admin"].includes(session.user.role)) return { error: "Reabrir um negócio GANHO desfaz receita reportada — só gestores podem." }
  // Multi-negócio: reabrir NÃO é mais barrado por outro aberto do contato (trava removida 2026-07-28).
  const now = new Date().toISOString()
...
export async function moveDeal(conversationId: string, dealId: string, stageId: string, reason?: string | null, note?: string | null, extras?: DealEventExtras): Promise<{ ok: true } | { error: string }> {
  ...
  if (status === "open") {
    await supabaseAdmin.from("chat_conversations")
      .update({ active_deal_id: dealId, updated_at: now }).eq("id", conversationId).eq("tenant_id", tenantId)
  } else if (conv.contact_id) {
    const nextActive = await nextOpenDealId(tenantId, conv.contact_id, dealId)
    await supabaseAdmin.from("chat_conversations")
      .update({ active_deal_id: nextActive, updated_at: now })
      .eq("tenant_id", tenantId).eq("contact_id", conv.contact_id).eq("active_deal_id", dealId)
  }
...
export async function cancelDeal(conversationId: string, dealId: string, reason?: string | null): Promise<{ ok: true } | { error: string }> {
  ...
  // Reaponta o foco pro próximo aberto do contato (ou null) em QUALQUER conversa que apontava
  // este negócio — multi-negócio: não deixar o card apontando um cancelado se há abertos.
  {
    const nextActive = await nextOpenDealId(tenantId, conv.contact_id, dealId)
    await supabaseAdmin.from("chat_conversations")
      .update({ active_deal_id: nextActive, updated_at: now })
...
export async function cancelDealById(dealId: string, reason?: string | null): Promise<{ ok: true } | { error: string }> {
  ...
  // Multi-negócio: reaponta o foco das conversas que apontavam este negócio pro próximo aberto (ou null).
  if (d.contact_id) {
    const nextActive = await nextOpenDealId(t, d.contact_id, dealId)
    await supabaseAdmin.from("chat_conversations")
      .update({ active_deal_id: nextActive, updated_at: now })
 ...
 *  • Multi-negócio: reabrir NÃO é barrado por outro aberto (a trava foi removida 2026-07-28).
 */
export async function reopenDeal(conversationId: string, dealId: string, opts?: { note?: string | null }): Promise<{ ok: true } | { error: string }> {
  ...
  const { data: deal } = await supabaseAdmin.from("tenant_deals")
    .select("contact_id, stage_id, pipeline_id, status, won_at, lost_at, lost_reason, canceled_at, assigned_to").eq("id", dealId).eq("tenant_id", tenantId).maybeSingle()
  const d = deal as { contact_id: string; stage_id: string | null; pipeline_id: string | null; status: string; won_at: string | null; lost_at: string | null; lost_reason: string | null; canceled_at: string | null; assigned_to: string | null } | null
  if (!d || d.contact_id !== conv.contact_id) return { error: "Negócio inválido para esta conversa" }
  if (d.status === "open") return { error: "Negócio já está aberto" }

  // Reabrir é write de GESTÃO → exige posse do negócio (dono/participante) ou gestor,
  // não só ver a conversa (fail-closed). O GANHO ainda exige gestor + justificativa abaixo.
  const scope = await getViewerScope()
  if (!canManageDealRow(scope, d)) return { error: "Só o dono/participante do negócio ou um gestor pode reabrir." }
