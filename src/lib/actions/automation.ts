...
export async function updateAutomationConfig(data: AutomationConfigInput): Promise<{ ok: true } | { error: string }> {
  ...
  const touchesWelcome = data.welcome_enabled !== undefined
    || data.welcome_message !== undefined
    || data.welcome_trigger !== undefined
    || data.welcome_reopen_days !== undefined
  const touchesBusinessHours = data.business_hours_enabled !== undefined
    || data.business_hours_message !== undefined
    || data.business_hours_schedule !== undefined
    || data.business_hours_timezone !== undefined

  try {
    if (touchesWelcome) await requireModule("welcome_message")
    if (touchesBusinessHours) await requireModule("business_hours")
  } catch {
    return { error: "Recurso não habilitado nesta conta." }
  }
...
export async function getAutomationConfig() {
  ...
  const [welcomeOn, businessHoursOn] = await Promise.all([
    hasModule(session.user.tenantId, "welcome_message"),
    hasModule(session.user.tenantId, "business_hours"),
  ])
  if (!welcomeOn && !businessHoursOn) return null
  ...
  if (!data) return null
  return {
    welcome_enabled:         welcomeOn ? data.welcome_enabled : false,
    welcome_message:         welcomeOn ? data.welcome_message : null,
    welcome_trigger:         welcomeOn ? data.welcome_trigger : "first_ever",
    welcome_reopen_days:     welcomeOn ? data.welcome_reopen_days : 30,
    business_hours_enabled:  businessHoursOn ? data.business_hours_enabled : false,
    business_hours_message:  businessHoursOn ? data.business_hours_message : null,
    business_hours_schedule: businessHoursOn ? data.business_hours_schedule : null,
    business_hours_timezone: businessHoursOn ? data.business_hours_timezone : "America/Sao_Paulo",
  }
}
