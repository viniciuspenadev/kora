...
export interface PipelineDashboardData {
  ...
  /** Unidades ATIVAS do tenant (dimensão do CRM). Vazio/1 = UI de unidade some. */
...
export async function getPipelineDashboard(opts: { pipelineId?: string | null; from: string; to: string }): Promise<PipelineDashboardData | { error: string }> {
  ...
  const [{ data: stageRows }, { data: dealRows }, { data: unitRows }] = await Promise.all([
    ...
    supabaseAdmin.from("tenant_deals")
      .select("id, name, status, estimated_value, stage_id, created_at, won_at, lost_at, lost_reason, stage_entered_at, assigned_to, unit_id, chat_contacts ( push_name, custom_name, profile_pic_url )")
      .eq("tenant_id", t).eq("pipeline_id", pipeline.id)
      .gte("created_at", opts.from).lte("created_at", opts.to)
      .order("created_at", { ascending: false }).limit(1000),
    supabaseAdmin.from("tenant_units")
      .select("id, name, color").eq("tenant_id", t).eq("active", true).order("name"),
  ])
  ...
  return {
    ...
    deals,
    agentProfiles,
    units: ((unitRows ?? []) as { id: string; name: string; color: string }[]),
  }
}
