...
vi.mock("@/lib/automation/auto-assign", () => ({
  assignNextAgent: async (...args: unknown[]) => { atribuicoes.push(args); return { assigned: true, agent_id: "bruno", reason: "ok" } },
  conversationHasOwner: (r: { assigned: boolean; reason?: string }) =>
    r.assigned || r.reason === "already_claimed" || r.reason === "already_assigned",
}))
...
function cenario(cfg: Record<string, unknown> = {}, convs = [conversa("c1")], runs: Record<string, unknown>[] = []) {
  ...
  db.seed("tenant_config", [{
    ...
    auto_assign_enabled: false, business_hours_enabled: false, business_hours_schedule: null,
...
describe("isolamento entre clientes", () => {
  ...
  it("🔑 a ESCRITA é escopada por tenant, não só pelo id da conversa", async () => {
    ...
    cenario({ inactivity_action: "redistribute" },
      [conversa("c1", { assigned_to: "ana" }), { ...conversa("c1"), tenant_id: "outro-tenant", assigned_to: "alheio" }])
    await runInactivitySweep()
    const linhas = db.tabelas.get("chat_conversations")!.filter((r) => r.id === "c1")
    expect(linhas.find((r) => r.tenant_id === T)!.assigned_to).toBeNull()
    expect(linhas.find((r) => r.tenant_id === "outro-tenant")!.assigned_to).toBe("alheio")
  })
})
...
describe("o que a rede FAZ, conforme o tenant escolheu", () => {
  ...
  it("'redistribuir' com a Distribuição DESLIGADA devolve pra fila", async () => {
    cenario({ inactivity_action: "redistribute", auto_assign_enabled: false },
      [conversa("c1", { assigned_to: "ana" })])
    await runInactivitySweep()
    expect(linha("c1").assigned_to).toBeNull()
    expect(atribuicoes).toHaveLength(0)
  })

  it("'redistribuir' com a Distribuição LIGADA chama o rodízio EXCLUINDO quem sumiu", async () => {
    cenario({ inactivity_action: "redistribute", auto_assign_enabled: true },
      [conversa("c1", { assigned_to: "ana" })])
    await runInactivitySweep()
    expect(atribuicoes[0]).toEqual([T, "c1", { exclude: ["ana"] }])
  })

  it("'devolver pra IA' religa a marca e solta o dono", async () => {
    cenario({ inactivity_action: "ai" }, [conversa("c1", { assigned_to: "ana", ai_handling: false })])
    await runInactivitySweep()
    expect(linha("c1").assigned_to).toBeNull()
    expect(linha("c1").ai_handling).toBe(true)
  })
})
