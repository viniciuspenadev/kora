...
describe("integração das portas e cobrança", () => {
  it("nenhuma das quatro portas mantém check-then-write em TypeScript", () => {
    ...
    expect(accept).toContain('rpc("aceitar_convite_cadastro_atomico"')
    expect(readFileSync("supabase/migrations/20260903000100_tenant_provisioning_atomic.sql", "utf8")).toContain('public.aceitar_convite_com_assento_atomico(p_token,p_name,p_password_hash)')
