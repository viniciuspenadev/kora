...
export async function findOrReopenConversation(
  ...
  if (resolved) {
    ...
    // Carteira: o retorno roteia pro DONO do cliente (owner_id no CONTATO), não pro
    // último handler — que pode ter sido um pit-stop (Ana do Financeiro). Fallback =
    // último assigned_to (sem dono comercial, o cliente volta pra quem atendeu).
