...
import { EvolutionProvider, MessageEditNotSentError } from "./evolution-provider"
...
const ack = { message: { editedMessage: { message: { protocolMessage: { type: 14, key,
  editedMessage: { conversation: "novo" }, timestampMs: Date.now() } } } } }
const response = (value: unknown) => new Response(JSON.stringify(value), { status: 200 })
afterEach(() => vi.unstubAllGlobals())

describe("Evolution edit transport", () => {
  it("uses original key in correct instance and validates acknowledgement", async () => {
    ...
    await expect(provider().editText("wa", "novo", Date.now() + 60_000)).resolves.toHaveProperty("editedAt")
    expect(fetch.mock.calls[0][0]).toBe("https://evolution.example/chat/findMessages/test-instance")
    expect(fetch.mock.calls[1][0]).toBe("https://evolution.example/chat/updateMessage/test-instance")
    expect(JSON.parse(fetch.mock.calls[1][1].body)).toEqual({ number: key.remoteJid, key, text: "novo" })
  })
  it.each([{ messages: { records: [] } }, { messages: { records: [{ key: { ...key, fromMe: false }, messageType: "conversation" }] } }])(
    "never sends without a verified original key", async value => {
      ...
      await expect(provider().editText("wa", "novo", Date.now() + 60_000)).rejects.toBeInstanceOf(MessageEditNotSentError)
  ...
  it("checks deadline after original lookup", async () => {
    ...
    await expect(provider().editText("wa", "novo", Date.now() - 1)).rejects.toBeInstanceOf(MessageEditNotSentError)
    expect(fetch).toHaveBeenCalledTimes(1)
  })
  it("does not classify a post-send error as safely retryable", async () => {
    ...
    await expect(provider().editText("wa", "novo", Date.now() + 60_000)).rejects.not.toBeInstanceOf(MessageEditNotSentError)
  ...
  it("does not accept an empty successful HTTP response as confirmation", async () => {
    ...
    await expect(provider().editText("wa", "novo", Date.now() + 60_000)).rejects.toThrow("Confirmação")
  })
})
