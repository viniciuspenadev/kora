...
interface CreateDealArgs {
  ...
  companyId?:      string | null   // âncora EMPRESA (F2) — negócio pode nascer só da empresa
  pipelineId:      string
  stageId:         string
...
export async function createDeal(args: CreateDealArgs): Promise<{ id: string } | { error: string }> {
  // 0. Âncora obrigatória: contato OU empresa (espelha o CHECK tenant_deals_anchor_chk).
  if (!args.contactId && !args.companyId) return { error: "Informe um contato ou uma empresa para o negócio" }

  // 1. Ownership: se veio contato, ele é do tenant?
  if (args.contactId) {
  ...
  // 1b. Se veio empresa, ela é do tenant? (anti-IDOR)
  if (args.companyId) {
    const { data: company } = await supabaseAdmin
      .from("tenant_companies").select("id")
      .eq("id", args.companyId).eq("tenant_id", args.tenantId)
      .maybeSingle()
    if (!company) return { error: "Empresa inválida para este tenant" }
  }
  ...
  // Multi-negócio (owner 2026-07-28): um contato pode ter VÁRIOS negócios abertos ao mesmo
  // tempo (ex: Site + Tráfego). A trava "1 aberto por contato" foi REMOVIDA. O `active_deal_id`
  // passa a apontar o MAIS RECENTE (o "em foco"); a sidebar do inbox lista os demais abertos.
  // NÃO recriar a trava.
  const status = dealStatus(args.isWon, args.isLost)

  const now = new Date().toISOString()

  // Carimbo de UNIDADE (dimensão do CRM): herda a unidade do VENDEDOR que abre o negócio.
  // Derivar em vez de perguntar. Congelado: reatribuir depois NÃO re-carimba. Caminhos
  // automáticos (sem `by` — automação/studio) nascem sem unidade (null).
  let unitId: string | null = null
  if (args.by) {
    const { data: creator } = await supabaseAdmin
      .from("tenant_users").select("unit_id")
      .eq("tenant_id", args.tenantId).eq("user_id", args.by).maybeSingle()
    unitId = (creator as { unit_id: string | null } | null)?.unit_id ?? null
  }
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      unit_id:             unitId,    // carimbo congelado (unidade do vendedor)
    })
    .select("id")
    .single()
  ...
  // cliente (fill-only-empty, ungated — abrir negócio é claim COMERCIAL). Só no CONTATO.
  if (args.contactId) await linkOwnerOnDeal(args.tenantId, args.contactId, args.by)
  ...
  if (!dealTable) {
    // Herda a tabela do CLIENTE — do contato, ou (negócio só-empresa) da empresa âncora.
    const { data: src } = args.contactId
      ? await supabaseAdmin.from("chat_contacts").select("price_table_id").eq("id", args.contactId).eq("tenant_id", args.tenantId).maybeSingle()
      : await supabaseAdmin.from("tenant_companies").select("price_table_id").eq("id", args.companyId!).eq("tenant_id", args.tenantId).maybeSingle()
    const inherited = (src as { price_table_id?: string | null } | null)?.price_table_id ?? null
  ...
  // Abrir negócio → contato vira Lead (ganho → Cliente). Nunca rebaixa. Doc §5. Só p/ contato.
  if (args.contactId) await syncContactLifecycleFromDeal(args.tenantId, args.contactId, { is_won: args.isWon, is_lost: args.isLost })
  return { id: dealId }
}
