...
function DealsCard({ conversationId, contactName, panel, onReload }: {
  ...
  // Multi-aberto: o "em foco" (activeDealId, ou o 1º aberto) é o hero rico; os demais
  // abertos viram linhas compactas; o histórico = só os fechados (won/lost/canceled).
  const active = panel.deals.find((d) => d.is_active && d.status === "open")
    ?? panel.deals.find((d) => d.status === "open") ?? null
  const openOthers = panel.deals.filter((d) => d.status === "open" && d.id !== active?.id)
  const rest       = panel.deals.filter((d) => d.status !== "open" && d.id !== active?.id)
  ...
  return (
    <Section icon={Briefcase} title="Negócios" action={panel.pipelines.length > 0 && (
      <button type="button" onClick={() => setShowNew(true)} aria-label="Novo negócio" title="Novo negócio"
      ...
      {openOthers.length > 0 && (
        <div className="mt-2.5">
          <p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
            Também em aberto ({openOthers.length})
          </p>
          <div className="space-y-1">
            {openOthers.map((d) => <OpenDealRow key={d.id} deal={d} />)}
          </div>
        </div>
      )}
...
// Linha compacta pra um negócio aberto que NÃO está em foco (multi-aberto): stage-dot +
// nome (truncate) + valor ancorado à direita + affordance de abrir a ficha (/negocios/[id]).
function OpenDealRow({ deal }: { deal: PanelDeal }) {
  const color = deal.stage?.color ?? "#64748b"
  const value = deal.estimated_value && deal.estimated_value > 0 ? dealBrl(Number(deal.estimated_value)) : null
  return (
    <Link href={`/negocios/${deal.id}`}
      className="group/od flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 hover:border-primary-200 hover:bg-primary-50/40 transition-colors">
      <span className="size-1.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
      <span className="min-w-0 flex-1 truncate text-[12px] font-medium text-slate-700 group-hover/od:text-slate-900 transition-colors">
        {deal.name?.trim() || "Negócio sem nome"}
      </span>
      {value && <span className="shrink-0 text-[12px] font-semibold text-slate-900 tabular-nums">{value}</span>}
      <ArrowUpRight className="size-3.5 shrink-0 text-slate-300 group-hover/od:text-primary-600 transition-colors" />
    </Link>
  )
}
