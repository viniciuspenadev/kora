...
import { NovaPropostaWizard } from "./nova-proposta-wizard"
...
export function DealsBoard({ pipelines, deals: initial, allTags, pipeId }: {
  ...
  const [addTarget, setAddTarget]   = useState<{ pipelineId: string; stageId: string; stageName: string } | null>(null)
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1">
        ...
        <div className="flex-1 min-h-0 flex gap-3 overflow-x-auto px-4 sm:px-6 py-3">
          {columns.map((stage) => (
            <DealColumn
              ...
              onAdd={() => setAddTarget({ pipelineId: pipeId, stageId: stage.id, stageName: stage.name })}
            />
          ))}
        </div>
      ...
      {addTarget && <NovaPropostaWizard target={addTarget} onClose={() => setAddTarget(null)} />}
