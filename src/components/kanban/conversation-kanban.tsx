  ...
  return (
    <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      ...
      <DragOverlay dropAnimation={null}>
        {activeCard ? <ConversationCard conv={activeCard} overlay deptById={deptById} hideSector={hideSector} /> : null}
      </DragOverlay>
...
function KanbanColumn({ col, list, tinted, readOnly, dragging, onAddConversation, deptById, hideSector }: {
  col: Column; list: Conversation[]; tinted: boolean; readOnly: boolean; dragging: boolean
  onAddConversation?: () => void
  deptById?: Map<string, { name: string; color: string }>; hideSector?: boolean
}) {
  ...
  return (
    <div
      ...
      className={`shrink-0 w-80 flex flex-col rounded-xl overflow-hidden transition-all duration-150 ${highlight ? "bg-primary-50 ring-2 ring-inset ring-primary-300" : tinted ? "" : "bg-slate-50/80"
        }`}
    >
      ...
      <div className="flex-1 min-h-0 px-2.5 pb-2 space-y-2.5 overflow-y-auto">
        {list.map((conv) => (
          <DraggableCard key={conv.id} conv={conv} readOnly={readOnly} deptById={deptById} hideSector={hideSector} />
        ))}
        {highlight && (
...
function DraggableCard({ conv, readOnly, deptById, hideSector }: { conv: Conversation; readOnly: boolean; deptById?: Map<string, { name: string; color: string }>; hideSector?: boolean }) {
...
function ConversationCard({
  conv, readOnly = false, dragRef, listeners, attributes, isDragging = false, overlay = false, deptById, hideSector = false,
}: {
  conv: Conversation
  showChannel?: boolean
  readOnly?: boolean
  ...
  const dept = conv.department_id ? deptById?.get(conv.department_id) ?? null : null

  const cardCls = `block group bg-white rounded-xl border transition-all ${overlay
    ...
    } ${isDragging ? "opacity-40 scale-[0.97]" : ""} ${aging && !overlay ? (aging.tone === "red" ? "border-l-[3px] border-l-red-400" : "border-l-[3px] border-l-amber-400") : ""
    }`
  ...
  const body = (
    <div className="p-3.5 space-y-2.5">
      ...
      {/* Meta: SETOR (tag como o inbox) · atendente · tempo. Na visão por-departamento
          (hideSector) a coluna já é o setor → cai no canal (evita redundância). */}
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 min-w-0">
        {hideSector ? (
          <span className="shrink-0">{ch.label}</span>
        ) : dept ? (
          <span className="inline-flex items-center gap-1 shrink-0 min-w-0 font-medium" style={{ color: dept.color }} title={`Setor: ${dept.name}`}>
          ...
          <span className="shrink-0 italic text-slate-300">sem setor · entrada</span>
        )}
        {ownerName && (
          <>
