...
import { layoutLanes, minutesInTz, minutesToLabel, snap15, SNAP_MIN, HOUR_PX, PX_PER_MIN, GRID_HOUR, GRID_HALF } from "./lanes"
...
export interface GridColumn {
  ...
  isToday: boolean
  appts: BoardAppt[]
  showWho: boolean            // semana-equipe → bolinha do recurso no card
...
export function TimeGrid({
  columns, startHour, endHour, now, onOpen, gestures, onSlotClick, colMinWidth = 160,
}: {
  columns: GridColumn[]
  startHour: number
  endHour: number
  ...
  const nowTop = (nowMin - gridStartMin) * PX_PER_MIN
  const nowVisible = nowMin >= gridStartMin && nowMin <= gridEndMin

  // Auto-scroll pra ~1h antes do "agora" ao montar.
  useEffect(() => {
    if (!scrollRef.current) return
    scrollRef.current.scrollTop = Math.max(0, (minutesInTz(new Date()) - gridStartMin) * PX_PER_MIN - HOUR_PX)
  ...
  const metaMap = useMemo(() => {
    ...
    for (const c of columns) {
      const occupied = [
        ...
        ...c.blackouts.map((b) => ({ id: b.id, startMin: b.startMin, durMin: b.durMin })),
      ]
  ...
  const { onPointerDown } = useGridGestures({
    ...
    gridStartMin, gridEndMin, reducedMotion,
  ...
  const gridBg = {
    backgroundImage:
      `repeating-linear-gradient(to bottom, ${GRID_HALF} 0 1px, transparent 1px ${HOUR_PX / 2}px),` +
      `repeating-linear-gradient(to bottom, ${GRID_HOUR} 0 1px, transparent 1px ${HOUR_PX}px)`,
  }
  ...
  function handleColClick(e: React.MouseEvent<HTMLDivElement>, c: GridColumn) {
    ...
    const raw = (e.clientY - rect.top) / PX_PER_MIN + gridStartMin
    const startMin = Math.max(gridStartMin, Math.min(gridEndMin - SNAP_MIN, snap15(raw)))
    const blocked = c.blackouts.find((b) => startMin >= b.startMin && startMin < b.startMin + b.durMin)
    ...
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
      ...
      <div ref={headerRef} className="overflow-hidden [scrollbar-gutter:stable] bg-canvas border-b border-slate-200">
        <div className="flex min-w-fit">
          ...
          {columns.map((c) => (
            <div key={c.key} className="flex-1 border-l border-slate-100 px-3 py-2" style={{ minWidth: colMinWidth }}>
              {c.header}
            </div>
          ))}
        </div>
      ...
      <div className="flex min-w-fit">
        ...
        <div className="w-14 shrink-0 sticky left-0 z-10 bg-canvas relative" style={{ height: totalH }}>
          {hours.map((h) => (
            <span key={h} className="absolute right-2 -translate-y-1/2 text-[10px] text-slate-400 tabular-nums" style={{ top: (h - startHour) * HOUR_PX }}>
              {minutesToLabel(h * 60)}
            </span>
          ))}
        </div>
        ...
        {columns.map((c) => {
          ...
          return (
            <div
              ...
              className={`flex-1 border-l border-slate-100 relative ${onSlotClick ? "cursor-copy" : ""}`}
              style={{ minWidth: colMinWidth, height: totalH, ...gridBg }}
            >
              {c.isToday && <div className="absolute inset-0 bg-primary-50/40 pointer-events-none" />}

              {/* Bloqueios (folga/feriado/manutenção) — atrás dos cards, não-interativos */}
              {c.blackouts.map((b) => {
                const vTop = (Math.max(b.startMin, gridStartMin) - gridStartMin) * PX_PER_MIN
                const vBottom = (Math.min(b.startMin + b.durMin, gridEndMin) - gridStartMin) * PX_PER_MIN
                if (vBottom <= vTop) return null
                return (
                  <div
                    key={b.id}
                    className="absolute left-[5px] right-[5px] rounded-lg border border-dashed border-slate-300 text-slate-400 text-[10.5px] font-semibold px-2 py-1.5 overflow-hidden pointer-events-none z-[1]"
              ...
              {c.appts.map((a) => (
                <AptCard key={a.id} a={a} pos={pos.get(a.id)} gridStartMin={gridStartMin} showWho={c.showWho} onOpen={onOpen} />
              ))}
