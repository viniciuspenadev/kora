...
// Botões de ação do header (§2.3): branco sobrepõe o canvas; azul = destaque.
const BTN_WHITE   = "inline-flex items-center gap-1.5 h-9 px-4 text-xs font-semibold bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-lg transition-colors shrink-0"
const BTN_PRIMARY = "inline-flex items-center gap-1.5 h-9 px-4 text-xs font-semibold bg-primary hover:bg-primary-700 disabled:opacity-50 disabled:pointer-events-none text-white rounded-lg transition-colors shrink-0"
      ...
      actions={
        <>
          {myResources.length > 0 && (
            <button type="button" onClick={() => setShareOpen(true)} className={BTN_WHITE}>
              <Share2 className="size-3.5" /> <span className="hidden md:inline">Compartilhar minha agenda</span><span className="md:hidden">Compartilhar</span>
            </button>
          )}
          {isAdmin && (
            <Link href="/agenda/configuracao" className={BTN_WHITE}>
              <Settings2 className="size-3.5" /> Configurar
            </Link>
          )}
          <button type="button" onClick={() => setPrefill({})} disabled={activeResources.length === 0} className={BTN_PRIMARY}>
          ...
          <div className="flex flex-wrap items-center gap-2 justify-between">
              ...
              {view !== "overview" && (
                <>
                  <div className="inline-flex items-center h-9 rounded-lg border border-slate-200 bg-white">
                    <button onClick={() => shift(-1)} className="size-9 grid place-items-center text-slate-500 hover:bg-slate-50 rounded-l-lg transition-colors"><ChevronLeft className="size-4" /></button>
                    ...
                    <button onClick={() => shift(1)} className="size-9 grid place-items-center text-slate-500 hover:bg-slate-50 rounded-r-lg transition-colors"><ChevronRight className="size-4" /></button>
                  </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="inline-flex items-center h-9 rounded-lg border border-slate-200 bg-white p-0.5">
                  {([["mine", "Minha"], ["all", "Todas"]] as const).map(([v, label]) => (
                    <button key={v} onClick={() => setScope(v)}
                      className={`h-full px-3 text-xs font-semibold rounded-md transition-colors ${scope === v ? "bg-primary-50 text-primary-700" : "text-slate-500 hover:text-slate-800"}`}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              {view !== "overview" && activeResources.length > 1 && (
                <div className="w-44"><SimpleSelect value={resourceFilter} onChange={setResourceFilter} className="h-9 text-xs"
